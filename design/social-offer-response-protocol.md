...
**Version:** 0.2.0 (revised per ChatGPT design review 2026-07-17: rejected v0.1.0 — ADR gate reversed from "no new ADR required" to an explicit ADR requirement (§11), and the pending-offer lifecycle reworked so cross-tick pending offers are actually reachable under the fixed seven-phase order (D3/D6))
**Phase:** Phase 3 — Stage 2 Slice 5
**Status:** Draft — awaiting ChatGPT re-review and Human approval (`ai-studio/workflows/feature-workflow.md` steps 4–7)
...
interface SocialOffer {
  ...
  readonly respondableAtTick: number;     // D3 — earliest resolution tick; stored, never re-derived from tuning on load
...
- **Response delay — the mechanism that makes pending state real (review revision):** an offer is never resolved on the tick that created it. Its earliest resolution tick is `respondableAtTick = createdAtTick + SOCIAL_OFFER_TUNING.responseDelayTicks`, with `responseDelayTicks >= 1` enforced as a structural minimum (a value of 0 would collapse the offer back into the same-tick auto-resolution v0.1.0 wrongly described — the delay's floor is architecture, its magnitude is tuning, DQ-2). This is the deterministic mechanism by which a `"pending"` offer persists across ticks: not an accident of preemption timing (v0.1.0's unreachable framing, correctly rejected by review — Phase 6 always follows Phase 5 within the same tick, so "Phase 6 never reaches the offer this tick" cannot occur under the fixed order), but a specified property of the protocol itself. The delay also carries the ADR-18 D5 meaning that the responder's answer is a *response*, not an instantaneous side effect of the initiator's own decision — and it is what makes save/load of a genuinely pending offer, timeout, and cancellation all reachable, testable behavior.
- **Phase 6 (Execution & consequences — M12), every tick:** every `"pending"` offer is examined in ascending `id` order:
  1. **Expiry check (D6):** if `clock.tick >= expiresAtTick`, resolve to `"expired"`. Stop.
  2. **Cancellation check (D6):** if the initiator's offer-creating goal has been abandoned or replaced (D6's precise condition), resolve to `"cancelled"`. Stop.
  3. **Hold check (D6):** if the initiator's offer-creating goal is currently *suspended* (a higher-tier interruption preempted it — decision-loop §2 trigger 2, goal-system suspend model), the offer stays `"pending"` untouched this tick. It resumes normal processing when the goal resumes; if the suspension outlasts `expiresAtTick`, step 1 expires it. Stop for this tick.
  4. **Not yet respondable:** if `clock.tick < respondableAtTick`, the offer stays `"pending"` untouched this tick. Stop for this tick.
  5. **Eligibility (D4):** compute responder eligibility. If ineligible, resolve to `"declined"` with the eligibility reason — no PRNG draw is spent on an offer that was never really open.
  6. **Acceptance draw (D5):** accepted → `beginExecution` runs exactly as today's `conversation`/`sharedDowntime` path (unchanged execution/consequence application in `execution.ts`/`tick.ts`). Declined → apply D6's decline consequence; no execution begins.
- **The initiator while an offer is pending:** the committed social goal stays active with no execution begun (the same "committed, awaiting resolution" posture a blocked goal's owner holds); `ambientStateFor` therefore reads the initiator as `blocked` during the delay window, which is Tier-1-honest — the colonist has an adopted intent the world has not yet answered. If review prefers a different texture, that is a presentation nuance inside the existing seven-state vocabulary (locked #29), not a protocol change.

This placement keeps the "six-trigger list stands, this is a scoping of trigger 2" framing ADR-18 D5 already committed to — no new trigger kind, no new phase, no change to §5's normative order. Every step above runs inside the existing Phase 5/Phase 6 slots; cross-tick behavior comes from the offer's own `respondableAtTick`/`expiresAtTick` fields being compared against the clock each tick, not from any reordering of phases.
...
- **Timeout:** `expiresAtTick = createdAtTick + SOCIAL_OFFER_TUNING.offerTimeoutTicks` (provisional constant, DQ-3), stored on the offer at creation and required to exceed `respondableAtTick` (an offer that would expire before it could ever be answered is malformed — D8 rejects it on load, and creation must never produce it; structurally this means `offerTimeoutTicks > responseDelayTicks`). At the start of Phase 6 resolution (D3 step 1), any `"pending"` offer with `state.clock.tick >= expiresAtTick` resolves to `"expired"` with `reason: "timeout"`, before all other checks. Expiry is a deterministic clock comparison, not a PRNG draw, and never touches Social crediting or relationship affinity (same non-effect as a decline, per D7 below) — it is not itself a change source in the ADR-12 table and applies no `applyInteraction` call. **Expiry's reachable path (review revision):** an offer expires when its initiator's goal is *suspended* (D3 step 3's hold) for long enough that the suspension outlasts `expiresAtTick` — e.g., a survival/critical-need interruption fires during the response-delay window and holds the goal suspended past the timeout. The hold rule deliberately neither cancels (the goal still exists and may resume) nor resolves (a suspended initiator is not present for the answer); timeout is precisely the bound on how long that limbo may last.
- **Initiator unavailable → cancel:** at D3 step 2, if the initiator's offer-creating goal has been **abandoned or replaced** — `currentGoal` is null, or neither `currentGoal.key` nor `suspendedGoal?.key` matches the goal key that created this offer — the offer resolves to `"cancelled"` with `reason: "initiator unavailable"`. This is distinct from suspension (D3 step 3): a suspended goal still exists and holds the offer pending; an abandoned/replaced goal no longer exists, so the offer it created must not resolve against it. Both conditions are pure reads of goal-stack state the tick already owns — no new trigger kind, no new state.
...
- **Load rejects** (same enumerated-rejection discipline as ADR-20 D8 — never silently repaired):
  ...
  - `respondableAtTick <= createdAtTick` (the response delay's structural `>= 1` floor, D3), or `expiresAtTick <= respondableAtTick` (an offer that expires before it is ever respondable is malformed, D6), or `resolvedAtTick` present and less than `createdAtTick`;
...
## 11. Architecture Review Gate — ADR Required (review revision)

`ai-studio/workflows/kanban-update-protocol.md`'s Architecture Review Required table lists **Data model**, **Save format**, and **Serialization** as ADR triggers. This design proposes all three: a new top-level `SimulationState.socialOffers` slice, a save-version bump, and new load-rejection rules. ChatGPT design review (2026-07-17) correctly rejected v0.1.0's claim that these could proceed without an ADR — the protocol's trigger table does not carve out an "implementation under an accepted ADR" exemption, and this design does not get to invent one.

**Determination (revised): an ADR is required before implementation.** The path, per `kanban-update-protocol.md`'s Architecture Review Required process and `feature-workflow.md` step 9's architecture-trigger rule:

1. This design document is the *input* to that ADR, not a substitute for it. After ChatGPT re-review and Human design approval of this document, a **candidate ADR (next free number, expected ADR-21 — "Social Offer State Storage")** is written in `ai-studio/adr/` from D1–D2 and D8–D9's storage/serialization decisions specifically: the `SocialOfferStore` shape, the persisted-counter id scheme, the bounded retention rule, the save-version bump, and the load-rejection list. The behavioral rules (D3–D7) remain governed by ADR-18 D5 and this design document — they instantiate accepted architecture and are not themselves ADR triggers.
2. The candidate ADR goes through the architecture workflow's own review and requires explicit Human acceptance, exactly as ADR-20 did under issue #109.
3. **Codex implementation of anything touching `SimulationState`, `serialization.ts`, or `replay.ts`'s `STATE_FIELDS` is blocked until that ADR is accepted.** The Kanban card for #120's implementation phase moves to `Blocked` if implementation is attempted before acceptance, per `feature-workflow.md` step 9's "Architecture trigger discovered: stop immediately" rule — except here the trigger is recognized at design time, so the ADR is sequenced *before* implementation begins rather than interrupting it.

What this design still asserts (unchanged, and for the ADR to confirm or revise): no *accepted* decision (ADR-01, ADR-05, ADR-12, ADR-18, ADR-20, or a locked freeze decision) is reopened, reinterpreted, or contradicted by D1–D9. The new ADR records a new decision in an empty slot (social-offer storage), in the same relationship to ADR-18 D5 that ADR-20 held to ADR-12 — the conceptual architecture existed; the storage decision needed its own accepted record.
...
| *(v0.1.0's rejected framing)* Same-tick resolution as the common case, with cross-tick pending arising only from Phase 5→6 preemption timing | No delay constant needed | ChatGPT review 2026-07-17: unreachable — Phase 6 always follows Phase 5 within the same tick under the fixed seven-phase order, so preemption "between" them cannot occur; pending state, timeout, and cancellation would all have been dead code. Replaced by D3's explicit `responseDelayTicks >= 1` mechanism |
| Narrow the design to avoid the ADR triggers entirely (no `SimulationState` field — e.g., offers as transient in-tick values never serialized) | Avoids the Data model / Save format / Serialization ADR gate | Directly violates Issue #120's acceptance criteria ("Save/load preserves pending and resolved offer state", "Replay compares offer/response state") — the criteria require persistent state, so the ADR gate is inherent to the task, not an artifact of this design's choices |
...
| DQ-2 | `SOCIAL_OFFER_TUNING.responseDelayTicks` magnitude (structural floor of 1 is D3's architecture; the value above the floor is pacing) | Prototype calibration (same discipline as ADR-18 DQ-18.2 — offer/response pacing) |
| DQ-3 | `SOCIAL_OFFER_TUNING.offerTimeoutTicks` (must exceed `responseDelayTicks` — D6's structural constraint) | Prototype calibration (same discipline as ADR-18 DQ-18.2 — offer/response pacing) |
| DQ-4 | `SOCIAL_OFFER_TUNING.resolvedOfferRetention` (bounded history window) | Prototype calibration, mirroring `RELATIONSHIP_TUNING.historyBound`'s existing provisional value |
...
| Offer created in Phase 5 (goal commit), resolved in Phase 6 (execution) no earlier than `respondableAtTick = createdAtTick + responseDelayTicks` with a structural floor of 1 tick | Preserves the unchanged seven-phase order (`design/engineering-specification.md` §5); makes pending state, timeout, cancellation, and save/load of pending offers reachable, deterministic behavior; matches ADR-18 D5's "scoping of trigger 2, not a new trigger kind" | Same-tick resolution with preemption-based cross-tick carry-over (v0.1.0 — rejected by review as unreachable under the fixed phase order); a dedicated new phase or trigger kind (reopens the frozen phase order without cause) |
| Suspended initiator goal *holds* the offer pending; abandoned/replaced goal *cancels* it; suspension outlasting `expiresAtTick` *expires* it | Distinguishes the three lifecycle exits by goal-stack facts the tick already owns (`currentGoal`/`suspendedGoal`), giving expiry a real reachable path instead of dead code | Treating suspension as cancellation (would kill offers across every survivable interruption, making timeout unreachable again); treating suspension as resolvable (a suspended initiator is not present for the answer) |
...
| *(Review revision)* ADR required before implementation: candidate ADR (expected ADR-21) covering the storage/serialization decisions (D1–D2, D8–D9), sequenced after design approval and before any Codex implementation | ChatGPT review 2026-07-17: the kanban protocol's Architecture Review Required table lists Data model / Save format / Serialization as ADR triggers with no exemption for work under an accepted parent ADR; v0.1.0's "no new ADR" claim contradicted the project's own workflow | v0.1.0's no-ADR determination (contradicted the trigger table); narrowing the design to dodge the triggers (contradicts Issue #120's own save/load and replay acceptance criteria — see §13) |
...
**Status:** Review — design v0.2.0 revised per ChatGPT design review 2026-07-17 (Rejected → revised), awaiting ChatGPT re-review and Human approval per `feature-workflow.md` steps 4–7. Codex does not implement until Human approval is posted to Issue #120 AND the §11 candidate ADR is accepted.
**Completed:** Produced `design/social-offer-response-protocol.md` v0.2.0 — offer state shape (D1), deterministic id/ordering scheme (D2), phase placement in the unchanged seven-phase tick with an explicit `responseDelayTicks >= 1` mechanism making pending offers genuinely cross-tick (D3), responder eligibility from snapshot facts only (D4), deterministic PRNG-attributed acceptance modulated by relationship state (D5), hold/cancel/expire lifecycle keyed to the initiator's goal-stack state (D6), unchanged downstream execution integration (D7), serialization/save-version/load-validation rules following ADR-20's precedent (D8), replay/inspector integration via existing generic mechanisms (D9), and an explicit Architecture Review Gate (§11) requiring a candidate ADR (expected ADR-21) covering the storage/serialization decisions before implementation.
**Changed Files:**
  CREATED  design/social-offer-response-protocol.md
**Validation:** Grounded against the current implementation directly — read `prototype/src/decision/goals.ts`, `task/tasks.ts`, `task/execution.ts`, `simulation/tick.ts`, `colonist/relationships.ts`, `core/serialization.ts` (conventions), `replay/replay.ts`, `inspection/inspector.ts` — every mechanism this design proposes (STATE_FIELDS addition, `detach()` reuse, `applyInteraction`/`isNonHostile` reuse, `nextSequence`-style vs. persisted-counter id reasoning, `currentGoal`/`suspendedGoal` reads for D6) is checked against what those modules already do, not assumed. Cross-checked against ADR-18 D4–D7, ADR-20 D1/D5/D7/D8, and `design/engineering-specification.md` v0.3.0's Stage 2 runtime boundary and seven-phase order for contradiction — none found. Both ChatGPT review findings (unreachable pending lifecycle; contradicted ADR trigger table) verified against the reviewed sources and resolved in D3/D6 and §11 respectively.
**Risks:** The acceptance-probability, response-delay, decline-friction, and timeout magnitudes (DQ-1–DQ-3) are provisional by design (prototype calibration territory, consistent with every other ADR-18/ADR-20 magnitude); if the reviewer wants the full ADR-18 D5 weighted composition now rather than the relationship-only subset, that requires either scope-expanding roster colonists' state (explicitly out of scope for this slice) or a documented exception — flagged here rather than decided unilaterally.
**Follow-up Tasks:** Write the §11 candidate ADR (expected ADR-21 — "Social Offer State Storage") after design approval, through the architecture workflow, before any implementation begins. No other follow-ups beyond what Issue #120 already tracks.
