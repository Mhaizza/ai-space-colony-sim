...
**Status:** Proposed (revision 5 after architecture review: trust classes, canonical projection identity, audit ownership, complete workflow-record contract, polling/failure semantics, database outage, runtime-mount boundaries, repository-qualified artifact references, per-repository startup probes, start_task supersession authorization, duplicate-JSON-member rejection, repository-scoped completeness partitions, repository-qualified path artifacts, the canonical RoleSlug registry, and Pull Request review startup probes closed)
**Date:** 2026-07-19
...
The canonical `RoleSlug` registry is closed and derived one-to-one from the governed role files under `ai-studio/roles/**/*.md` (excluding `README.md`), using each file's basename without extension as its slug. Its initial closed set is: `ai-simulation-designer`, `creative-director`, `game-systems-designer`, `gameplay-engineer`, `qa-reviewer`, `technical-director`, `ui-ux-engineer`, `world-designer`. A `role` value outside this set is unknown and quarantined. The set grows only when a new role file is added under `ai-studio/roles/` and the workflow-compatibility prerequisite card (below) generates and validates a canonical machine-readable manifest (e.g. a generated `role-slugs.json`) from those files; templates, the principal registry, and the parser must all read that manifest rather than independently deriving slugs, so they cannot disagree about which roles are valid.
...
- `RoleSlug` registry tests: every initial closed slug is accepted, a slug not in the registry is quarantined, and the generated canonical manifest matches the current `ai-studio/roles/**/*.md` file set exactly (fails the workflow-compatibility prerequisite card's own validation if it drifts).
...
- The canonical `RoleSlug` registry's derivation rule changes, or a role is accepted without a matching file under `ai-studio/roles/`.
...
**Status:** Review - ADR status Proposed; revision 5 corrects the invalid revision-4 Approved verdict and closes the five findings posted before that verdict (repository-scoped `github_issue`/CI partitions, repository-qualified `path:` artifacts, the canonical `RoleSlug` registry, and Pull Request review startup probes); awaiting architecture re-review and Human acceptance.
**Completed:** Drafted ADR-23 from the accepted Mission Control design v0.4.0 and revised D3-D5 after the Architecture Review Correction on Issue #142: `github_issue`/`github_pull_request` completeness partitions now use the owning repository node ID instead of a fixed `self` repository; every CI-derived partition (`github_check_suite`, `github_check_run`, `github_workflow_run`, `github_commit_status`) includes the owning repository node ID so a shared commit OID across `self`/`mission-control` cannot cross-tombstone; `path:` artifacts are now repository-qualified (`path:<RepoQualifier>#<path>`) under the same mapping/probe gating as `pr:`/`issue:`; `RoleSlug` now resolves against a closed canonical registry derived from `ai-studio/roles/**/*.md`, with an explicit initial slug set and a generated-manifest prerequisite instead of an informal reference; D5's startup probe checklist now covers Pull Request reviews for every enabled `RepoQualifier`. D1/D2/D6-D8, credential scope, Windows host export, local deployment, and the future-control boundary remain otherwise unchanged.
**Changed Files:**
  CREATED  ai-studio/adr/0023-mission-control-projection-and-control-boundary.md
**Validation:** Traced every decision to design v0.4.0, Issue #142, the Architecture Workflow, or an explicit architecture-review correction; confirmed no gameplay ADR is reopened; confirmed the existing OpenClaw runtime and all implementation files remain untouched. Added required tests for trust-class impersonation/overlap, source identity/partitions, polling/rate-limit behavior, full record-matrix/precedence, audit-loss reporting, database outage, runtime mount isolation, repository-qualifier validation, per-qualifier startup probes, start_task supersession authorization, duplicate-JSON-member rejection, repository-scoped partition collision, `RoleSlug` registry/manifest drift, and Pull Request review probe coverage.
