...
**Status:** Proposed (revision 10 after architecture review: trust classes, canonical projection identity, audit ownership, complete workflow-record contract, polling/failure semantics, database outage, runtime-mount boundaries, repository-qualified artifact references, per-repository startup probes, start_task supersession authorization, duplicate-JSON-member rejection, repository-scoped completeness partitions, repository-qualified path artifacts, the canonical RoleSlug registry, Pull Request review startup probes, the reserved human-owner RoleSlug, pinned public fork visibility, local-auth redaction, self-repository-only workflow-record authority, deterministic kanban_update/completion worker-role pairing, canonical PR-conversation-comment partitioning, the manual-refresh action-route exception, scoped worker-role pairing ban, GitHub-credential audit/diagnostic redaction, Worker-class-only assignment targets, typed non-PR review/approval gates, inline PR review comment projection, PR-head-match validation, and independent handoff-target validation closed)
**Date:** 2026-07-19
...
|---|---|---|---|---|---|---|
| `start_task` | Worker identity being started (registered Worker-class principal only; never `human` or `chatgpt-reviewer`) | required governed role | `null` | `null` | `null` | `null`, or (only when authored by the Human approver) a prior same-card assignment record |
| `handoff` | next Worker identity (registered Worker-class principal only; never `human` or `chatgpt-reviewer`) | next governed role | `null` | `null` | `null` | required effective assignment record |
| `review_result` | `chatgpt-reviewer` | required reviewer role | required `pr:` (PR gate) or `path:` (non-PR design/architecture gate) reference; never `issue:` | required exact head — PR's current head for `pr:`, exact reviewed commit for `path:` | required review result | `null` or prior same-card review result |
| `human_approval` | `human` | required, exactly `human-owner` | required `pr:` (PR gate) or `path:` (non-PR design/architecture gate) reference; never `issue:` | required exact head — PR's current head for `pr:`, exact reviewed commit for `path:` | `null` | `null` or prior same-card Human approval |
...
- `review_result` is valid only from the disjoint Reviewer principal, carries `approved` or `revisions_required`, and identifies either: a `pr:` artifact whose `head` is the referenced Pull Request's exact current head commit SHA as observed by sync at or immediately after the record's authoring — a well-formed but non-current or anticipatory SHA is quarantined, and a record's validity is fixed at authoring, never later satisfied by the Pull Request subsequently reaching that SHA; or a `path:` artifact whose `head` is the exact repository commit SHA at which the reviewed document version exists — inherently exact and not subject to a current-head check, since it names one fixed historical commit rather than a moving branch.
- `human_approval` is valid only from the disjoint Human approver principal, carries `role` exactly `human-owner`, and identifies either a `pr:` artifact under the same exact-current-head rule as `review_result`, or a `path:` artifact under the same exact-historical-commit rule as `review_result`.
- `review_result` and `human_approval` represent two gate kinds: implementation Pull Request gates (`pr:` artifact) and non-PR design/architecture gates (`path:` artifact) — such as this ADR's own architecture-review and Human-acceptance verdicts on Issue #142, which reference `path:self#ai-studio/adr/0023-mission-control-projection-and-control-boundary.md` at each revision's exact head. An `issue:` artifact is never valid for either type: a gate always reviews one exact versioned document (a PR's head, or a `path:`'s exact commit), never an abstract Issue with no fixed version to pin.
...
- Non-PR gate typed-record tests: a `review_result`/`human_approval` with a `path:` artifact and exact reviewed-commit `head` is accepted and appears as a structured Approvals-panel gate; the same record with an `issue:` artifact is quarantined; a design/architecture verdict posted as prose only (no machine record) remains Live Feed only, never inferred as a structured gate.
- PR-head-match tests: a `review_result`/`human_approval` naming a `pr:` artifact with a well-formed 40-character `head` that is not the referenced Pull Request's current head at authoring is quarantined; the same record does not later become effective if the Pull Request subsequently reaches that SHA (validity is fixed at authoring, not re-evaluated); a record whose `head` does match the current head at authoring is accepted, and stops being effective the moment the PR's head changes.
...
- The `pr:`-artifact current-head match rule for `review_result`/`human_approval` changes, or a record's validity is ever re-evaluated against a later head than the one observed at authoring.
...
| `review_result`/`human_approval` naming a `pr:` artifact must carry the referenced Pull Request's exact current head at authoring; validity is fixed then, never re-satisfied later | Requiring only "an exact Pull Request plus full SHA" let a mistyped or prewritten approval for a future head later become valid once the PR happened to reach that SHA | Re-evaluating head-match at read time instead of authoring time (would let prewritten approvals eventually satisfy the gate) |
...
**Status:** Review - ADR status Proposed; revision 10 closes all four unresolved P2 findings from the revision-9 re-review (typed non-PR review/approval gates, inline PR review comment projection, PR-head-match validation, and independent handoff-target validation); awaiting a fresh architecture review of this exact head.
**Completed:** Drafted ADR-23 from the accepted Mission Control design v0.4.0 and revised D3/D4/D5 after the revision-9 re-review. `review_result`/`human_approval` now support `path:` artifacts for non-PR design/architecture gates (never `issue:`), reversing the revision-8/9 PR-only restriction to match design v0.4.0's typed-reviewer-outcomes decision — this ADR's own architecture-review/Human-acceptance flow on Issue #142 is itself now representable as `path:self#ai-studio/adr/0023-mission-control-projection-and-control-boundary.md` at each revision's exact head. A `pr:`-artifact `review_result`/`human_approval` must now carry the referenced Pull Request's exact current head observed at authoring, fixed then and never later satisfied by the PR reaching that SHA. D3 adds `github_pull_request_review_comment` as its own source type, partitioned under its parent Pull Request, with a matching D5 startup probe. D4's `handoff` target validation is now independent of the authoring principal's own declarable-identity allowlist (which continues to govern only `start_task`'s self-declaration), so ordinary cross-worker handoffs are always possible without granting cross-declaration rights that could be reused for unauthorized `start_task` self-assignment. Required Invariants 5/6/7, validation requirements, revisit triggers, and the Decision Log were updated to match, including revising the now-superseded revision-8/9 Decision Log row about PR-only gate scoping. D1/D2/D6/D7/D8 and the rest of D3/D4/D5 remain unchanged from revision 9.
**Changed Files:**
  CREATED  ai-studio/adr/0023-mission-control-projection-and-control-boundary.md
**Validation:** Traced every decision to the four revision-9 re-review findings explicitly scoped by the requester and to design v0.4.0's typed-reviewer-outcomes decision log entry; confirmed no gameplay ADR is reopened; confirmed the existing OpenClaw runtime and all implementation files remain untouched. Added non-PR gate typed-record tests, PR-head-match tests, a handoff-target validation test, and `github_pull_request_review_comment` source tests, alongside all tests carried from revisions 1-9.
