...
**Status:** Proposed (revision 9 after architecture review: trust classes, canonical projection identity, audit ownership, complete workflow-record contract, polling/failure semantics, database outage, runtime-mount boundaries, repository-qualified artifact references, per-repository startup probes, start_task supersession authorization, duplicate-JSON-member rejection, repository-scoped completeness partitions, repository-qualified path artifacts, the canonical RoleSlug registry, Pull Request review startup probes, the reserved human-owner RoleSlug, pinned public fork visibility, local-auth redaction, self-repository-only workflow-record authority, deterministic kanban_update/completion worker-role pairing, canonical PR-conversation-comment partitioning, the manual-refresh action-route exception, non-PR review/approval gate scoping, scoped worker-role pairing ban, GitHub-credential audit/diagnostic redaction, and Worker-class-only assignment targets closed)
**Date:** 2026-07-19
...
**Status:** Review - ADR status Proposed; revision 9 closes the remaining three unresolved P2 findings from the revision-7 re-review (worker/role-pairing-ban scope, GitHub-credential audit/diagnostic redaction, Worker-class-only assignment targets); all six findings from that re-review round are now closed; awaiting a fresh architecture review of this exact head.
**Completed:** Drafted ADR-23 from the accepted Mission Control design v0.4.0 and revised D4/D5 after the revision-8 re-review: Required Invariant 5's worker/role pairing clause is now explicitly scoped to `kanban_update`/`completion` only, so it can no longer be read to affect `start_task`, `handoff`, or `review_result`; D5's credential-redaction requirement now explicitly names operational audit records and diagnostic output alongside browser code, API output, projection rows, quarantine payloads, and logs; D4 now constrains `start_task`/`handoff` assignment targets to registered Worker-class `WorkerId` members, structurally excluding the `human` and `chatgpt-reviewer` sentinels from ever holding a card's effective assignment. Required Invariants 5/7, validation requirements, revisit triggers, and the Decision Log were updated to match. D1/D2/D3/D6/D7/D8 and the rest of D4/D5 remain unchanged from revision 8.
**Changed Files:**
  CREATED  ai-studio/adr/0023-mission-control-projection-and-control-boundary.md
**Validation:** Traced every decision to the three remaining revision-7 re-review findings explicitly scoped by the requester; confirmed no gameplay ADR is reopened; confirmed the existing OpenClaw runtime and all implementation files remain untouched. Added a worker/role-pairing-scope test, a credential-redaction-coverage test, and an assignment-target-principal test, alongside all tests carried from revisions 1-8.
