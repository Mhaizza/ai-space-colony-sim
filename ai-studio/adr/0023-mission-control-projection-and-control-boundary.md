...
**Status:** Proposed (revision 6 after architecture review: trust classes, canonical projection identity, audit ownership, complete workflow-record contract, polling/failure semantics, database outage, runtime-mount boundaries, repository-qualified artifact references, per-repository startup probes, start_task supersession authorization, duplicate-JSON-member rejection, repository-scoped completeness partitions, repository-qualified path artifacts, the canonical RoleSlug registry, Pull Request review startup probes, the reserved human-owner RoleSlug, pinned public fork visibility, local-auth redaction, and self-repository-only workflow-record authority closed)
**Date:** 2026-07-19
...
- `RoleSlug` registry tests: every initial closed slug (including reserved `human-owner`) is accepted, a slug not in the registry is quarantined, `human_approval` with `role` other than `human-owner` is quarantined, `human-owner` is rejected for every non-`human_approval` record type, and the generated canonical manifest matches the current `ai-studio/roles/**/*.md` file set plus the reserved entry exactly (fails the workflow-compatibility prerequisite card's own validation if it drifts).
...
- The `human-owner` slug's binding to `human_approval` changes, or any other record type accepts it.
...
| Reserved `human-owner` `RoleSlug`, valid only for `human_approval` | The file-derived registry has no Human-owner slug; leaving the row as informal prose lets implementers quarantine every Human approval or misuse an unrelated slug | Leaving "Human-owner role" undefined; reusing an existing role file's slug (e.g. `technical-director`) to mean Human approval |
...
**Status:** Review - ADR status Proposed; revision 6 closes four new findings from the revision-5 re-review (the reserved `human-owner` RoleSlug, pinned public D1 fork visibility, D7 local-auth redaction, and self-repository-only workflow-record authority); awaiting architecture re-review and Human acceptance.
**Completed:** Drafted ADR-23 from the accepted Mission Control design v0.4.0 and revised D1/D4/D5/D7 after the revision-5 re-review: `RoleSlug`'s canonical registry now includes a reserved `human-owner` slug used only (and required) by `human_approval`, closing the previously undefined "Human-owner role" prose; D1 pins the Mission Control fork's GitHub visibility as public, with D5 restated to depend structurally on that and an explicit escalation instruction if it ever becomes unacceptable; D7 now carries its own explicit local-auth token/header/cookie redaction rule independent of D5's GitHub-credential redaction; D4 now states that authoritative `ai-workflow-record:v1` records require an Issue in `Mhaizza/ai-space-colony-sim` linked to Project #4, and a same-numbered `mission-control` Issue is Live Feed only. D2/D3/D6/D8 and the rest of D1/D4/D5/D7 remain otherwise unchanged.
**Changed Files:**
  CREATED  ai-studio/adr/0023-mission-control-projection-and-control-boundary.md
**Validation:** Traced every decision to design v0.4.0, Issue #142, the Architecture Workflow, or an explicit architecture-review correction; confirmed no gameplay ADR is reopened; confirmed the existing OpenClaw runtime and all implementation files remain untouched. Added required tests for trust-class impersonation/overlap, source identity/partitions, polling/rate-limit behavior, full record-matrix/precedence, audit-loss reporting, database outage, runtime mount isolation, repository-qualifier validation, per-qualifier startup probes, start_task supersession authorization, duplicate-JSON-member rejection, repository-scoped partition collision, `RoleSlug` registry/manifest drift, Pull Request review probe coverage, `human-owner` slug binding, fork-visibility, and local-auth redaction.
