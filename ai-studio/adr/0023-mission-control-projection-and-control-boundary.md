...
**Status:** Proposed (revision 7 after architecture review: trust classes, canonical projection identity, audit ownership, complete workflow-record contract, polling/failure semantics, database outage, runtime-mount boundaries, repository-qualified artifact references, per-repository startup probes, start_task supersession authorization, duplicate-JSON-member rejection, repository-scoped completeness partitions, repository-qualified path artifacts, the canonical RoleSlug registry, Pull Request review startup probes, the reserved human-owner RoleSlug, pinned public fork visibility, local-auth redaction, self-repository-only workflow-record authority, and deterministic kanban_update/completion worker-role pairing closed)
**Date:** 2026-07-19
...
**Status:** Review - ADR status Proposed; revision 7 closes one internal contradiction found by manual consistency review after revision 6 (deterministic `kanban_update`/`completion` worker-role pairing); awaiting a fresh architecture review of this exact head.
**Completed:** Drafted ADR-23 from the accepted Mission Control design v0.4.0 and revised D4 after manual consistency review: revision 6 reserved `human-owner` exclusively for `human_approval`, but the matrix still permitted `worker = human` for `kanban_update`/`completion` with "required corresponding role" — leaving no deterministic valid role for those two Human-authored record types. `kanban_update`/`completion` now require `role` exactly `human-owner` when `worker` is `human`, and exactly the card's effective assignment record's role when `worker` is the effective Worker identity; any other worker/role pairing for these two types is malformed and quarantined. `human-owner` remains illegal for `start_task`, `handoff`, and `review_result`. D1/D2/D3/D5/D6/D7/D8 and the rest of D4 remain unchanged from revision 6.
**Changed Files:**
  CREATED  ai-studio/adr/0023-mission-control-projection-and-control-boundary.md
**Validation:** Traced the decision to the revision-6 text itself (an internal contradiction, not a new external finding) and to design v0.4.0's Human/Worker authorship rules; confirmed no gameplay ADR is reopened; confirmed the existing OpenClaw runtime and all implementation files remain untouched. Added worker/role pairing tests for `kanban_update`/`completion` (Human-authored, Worker-authored, and every mismatched pairing) alongside all tests carried from revisions 1-6.
