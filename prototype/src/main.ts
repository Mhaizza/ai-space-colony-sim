...
export function verifySaveReplay(save: string, seed: number): ReplayResult {
  ...
  // Stage 2 Slice 6b: every colonist is fully simulated, so createInitialState's primary/roster
  // parameter split is purely a call-shape artifact — either produces the identical collection
  // membership. The canonically-first entry is used as "primary" arbitrarily but consistently.
